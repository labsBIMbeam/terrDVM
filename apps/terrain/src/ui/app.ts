...
/**
 * The terrain napplet: select a bounding box on a map, ask what data exists
 * there, generate an honest 3D model of it, inspect and export it. One app,
 * one job — the presence, placement and avatar layer lives in the player
 * napplet, and the field-measurement protocol in its own.
 */
...
export function renderApp(root: HTMLDivElement, options: RenderAppOptions = {}): void {
  ...
  if (!mapCanvas || !stateAlert || !announcement || !emptyState || !helper || !areaReadout ||
      ...
      !jobArea || !demoButton || !jobScoreDot || !jobScoreValue ||
    ...
    throw new Error('Incomplete terrCVM terrain UI scaffold.');
  }
  ...
  // The region's curated demo selection: a prewarmed bbox that goes straight
  // to generation.
